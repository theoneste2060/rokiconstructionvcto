...
type Section = "overview" | "content" | "analytics" | "submissions";
...
function Admin() {
  ...
  if (!stats?.authorized) {
    return (
      <div className="min-h-dvh flex items-center justify-center bg-gray-50 dark:bg-dark-bg px-4">
...
// ---------------------------------------------------------------------------
// Dashboard shell: collapsible sidebar + top bar
// ---------------------------------------------------------------------------
...
function Dashboard({ stats, onLogout }: { stats: AdminStats; onLogout: () => void }) {
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${
              ...
              active
                ...
                : "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-primary dark:hover:text-primary-light"
            }`}
          >
            <span className="shrink-0">{item.icon}</span>
            {!collapsed && <span>{item.label}</span>}
            {!collapsed && item.id === "submissions" && stats.submissions.length > 0 && (
              <span className="ml-auto px-2 py-0.5 text-xs font-semibold rounded-full bg-accent/15 text-accent dark:text-accent-light">
  ...
  const sidebarFooter = (
    <div className="px-3 pb-4 space-y-1 border-t border-gray-200 dark:border-gray-800 pt-4">
      <Link
        to="/"
        title={collapsed ? "View Website" : undefined}
        className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/60 hover:text-primary dark:hover:text-primary-light transition-colors ${
          collapsed ? "justify-center" : ""
        }`}
      >
      ...
      <button
        ...
        className={`hidden md:flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800/60 transition-colors ${
          collapsed ? "justify-center" : ""
        }`}
      >
        <svg
  ...
  return (
    <div className="min-h-dvh flex bg-gray-50 dark:bg-dark-bg">
      {/* ===== Sidebar (desktop) ===== */}
      <aside
        className={`hidden md:flex flex-col shrink-0 sticky top-0 h-dvh bg-white dark:bg-dark-card border-r border-gray-200 dark:border-gray-800 transition-[width] duration-300 ${
          collapsed ? "w-[76px]" : "w-64"
        }`}
      >
        <div className={`h-16 flex items-center border-b border-gray-200 dark:border-gray-800 ${collapsed ? "justify-center px-2" : "px-5"}`}>
          {collapsed ? (
            <img src="/images/roki-logo.png" alt="ROKI" className="h-9 w-9 rounded-lg bg-white p-[3px] ring-1 ring-black/10 dark:ring-white/10" />
          ) : (
            <Link to="/">
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 flex flex-col bg-white dark:bg-dark-card border-r border-gray-200 dark:border-gray-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* ===== Top bar ===== */}
        <header className="sticky top-0 z-40 h-16 flex items-center justify-between gap-4 px-4 sm:px-6 bg-white/90 dark:bg-dark-card/90 backdrop-blur-md border-b border-gray-200 dark:border-gray-800">
          {/* Left: menu toggle + logo (logo shown here on mobile where sidebar is hidden) */}
          <div className="flex items-center gap-3 min-w-0">
            ...
            <button
              onClick={() => setCollapsed(!collapsed)}
              className="hidden md:inline-flex p-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
            ...
            <h1 className="hidden md:block text-lg font-bold text-gray-900 dark:text-white font-display truncate">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3">
            ...
            <div className="relative">
              ...
              >
                ...
                {stats.submissions.length > 0 && (
                  <span className="absolute top-1 right-1 min-w-[18px] h-[18px] px-1 flex items-center justify-center text-[10px] font-bold text-white bg-accent rounded-full">
                    {stats.submissions.length > 9 ? "9+" : stats.submissions.length}
                  </span>
                )}
              </button>
              ...
              {openMenu === "bell" && (
                <>
                  <div className="absolute right-0 mt-2 w-80 z-50 rounded-2xl bg-white dark:bg-dark-card border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-800 font-semibold text-sm text-gray-900 dark:text-white">
                      Notifications
                    </div>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 p-1.5 sm:pl-2 sm:pr-3 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
              ...
              >
                ...
                <span className="hidden sm:block text-left">
                  <span className="block text-sm font-semibold text-gray-900 dark:text-white leading-tight">Administrator</span>
                  <span className="block text-[11px] text-gray-500 dark:text-gray-400 leading-tight">ROKI Construction</span>
                </span>
                <svg
                  className={`hidden sm:block w-4 h-4 text-gray-400 transition-transform duration-200 ${openMenu === "profile" ? "rotate-180" : ""}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
                ...
                <>
                  <div className="absolute right-0 mt-2 w-56 z-50 rounded-2xl bg-white dark:bg-dark-card border border-gray-200 dark:border-gray-800 shadow-xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-800">
                      ...
                      <div className="text-xs text-gray-500 dark:text-gray-400">Site dashboard access</div>
                    </div>
                    <Link
                      to="/"
                      onClick={closeMenus}
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8">
          ...
          {section === "content" && (
            <ContentEditor password={typeof sessionStorage !== "undefined" ? sessionStorage.getItem(STORAGE_KEY) ?? "" : ""} />
          )}
